...
resource "docker_volume" "bgg-data-vol" {
    name = "${var.app_namespace}-bgg-data-vol"    
}
...
resource "docker_container" "bgg-database" {
    ...
    volumes {
        volume_name = docker_volume.bgg-data-vol.name
...
resource "docker_container" "bgg-backend" {
    ...
    env = [
        ...
        "BGG_DB_HOST${err.bgg-database.name}",
    ]
...
resource "local_file" "root-at-nginx" {
